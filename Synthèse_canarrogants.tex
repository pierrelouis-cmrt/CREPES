...
\begin{itemize}
    ...
    \item \code{PROVENANCE.md} et \code{THEORIE.md} : traçabilité et rappels théoriques.
\end{itemize}
...
\begin{itemize}
    \item la \motcle{convection forcée} issue du travail des Chevreaux ;
    \item la \motcle{convection naturelle} issue du travail des Ornithorynquietant.
\end{itemize}

Par défaut, les deux sont actives avec un vent constant de $2.5\ \mathrm{m\,s^{-1}}$, sans appel réseau. Elles peuvent être coupées avec :
...
Les données qui ne sont pas générées par code restent des \motcle{sources locales} à fournir ou remplacer manuellement, par exemple certains fichiers CERES, RZSM ou shapefiles.

\partietitre{5/Modules conservés}
\addcontentsline{toc}{subsection}{Modules conservés}

Certains modules sont gardés sans être totalement intégrés au moteur principal. C'est notamment le cas de \code{codes\_python/physique/diffusion.py}, conservé pour une reprise future, mais non branché car le flux de surface reste ambigu.

Les gaz à effet de serre du modèle 0 ne sont pas intégrés dans cette maintenance. La nouvelle modélisation du \COtwo{} est traitée séparément dans la partie II.
...
L'objectif général est de modéliser l'effet du \COtwo{} présent dans l'atmosphère sur les flux radiatifs reçus et émis par la Terre.
...
\begin{tcolorbox}
\motcle{Point important}

On ne cherche pas à imposer directement une relation du type $T=f(\COtwo)$. Le \COtwo{} agit d'abord sur \motcle{l'opacité infrarouge}. Cette opacité modifie les flux radiatifs. La température vient ensuite, via un bilan d'énergie.
\end{tcolorbox}
...
On distingue les \motcle{couches} A, B, C etc, qui contiennent la matière atmosphérique, et les \motcle{interfaces}, où l'on suit les flux.
...
Le modèle sépare deux types de rayonnement :

\begin{itemize}
    \item le rayonnement solaire visible, qui vient du Soleil ;
    \item le rayonnement infrarouge, qui est émis par la Terre et par les couches atmosphériques.
\end{itemize}

Dans cette première modélisation, les couches atmosphériques sont supposées \motcle{transparentes au visible}. Le rayonnement solaire traverse donc l'atmosphère. Dans l'infrarouge, en revanche, les couches peuvent absorber, transmettre et réémettre une partie du flux.
...
\begin{itemize}
    \item $S_0$ l'irradiance solaire, en $\fluxunit$ ;
...
Cette loi donne un \motcle{flux total}. Pour connaître la part de ce flux dans une bande absorbante du \COtwo{}, il faut utiliser la loi de Planck.
...
Le facteur $\pi$ vient de l'intégration sur l'hémisphère d'émission. Il permet de passer d'une luminance spectrale à un flux hémisphérique.
...
Le modèle 1 est une première version simple du modèle général. Il vérifie la chaîne de calcul : émission de la surface, absorption par le \COtwo{}, transmission, réémission, flux montant et flux descendant.
...
\begin{itemize}
    \item il utilise exactement trois couches atmosphériques ;
    \item les trois couches ont la même température ;
    \item les trois couches ont la même concentration en \COtwo{} ;
    \item seules deux bandes infrarouges du \COtwo{} sont traitées explicitement ;
    \item les absorbances des bandes sont fixées à l'avance et ne sont pas recalculées pendant l'exécution ;
    \item les températures restent constantes : le modèle calcule des flux, pas une évolution thermique.
\end{itemize}

\bloc{Valeurs numériques utilisées}

Les valeurs sont regroupées ici pour éviter d'étaler les constantes sur plusieurs pages :
...
Le premier résultat est le \motcle{flux infrarouge sortant} au sommet de l'atmosphère. Le second est le \motcle{flux infrarouge descendant} reçu par la surface à cause de la réémission des couches.
