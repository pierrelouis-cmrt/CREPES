...
def atmosphere_standard(altitude_geometrique_m: np.ndarray,pression_surface_pa: float = PRESSION_SURFACE_STANDARD_PA,temperature_surface_k: float = TEMPERATURE_SURFACE_STANDARD_K,) -> tuple[np.ndarray, np.ndarray]:
    ...
    "atmosphere_standard(liste(altitude_geometrique_m),pression_surface_pa,temperature_de_la_surface_k)->tableau (liste(température),liste(pression))"
...
def altitude_depuis_pression(pression_pa: float,pression_surface_pa: float = PRESSION_SURFACE_STANDARD_PA,temperature_surface_k: float = TEMPERATURE_SURFACE_STANDARD_K,) -> float:
    ...
    "altitude_depuis_pression(pression_pa,pression_surface_pa,temperature_de_la_surface_k)->l'altitude geometrique en metres"
...
def temperature_moyenne_altitude(altitude_bas_m: float,altitude_haut_m: float,nombre_points: int = 1_001) -> float:
    ...
    "temperature_moyenne_altitude(altitude_bas_m,altitude_haut_m,nombre_points_pr_moyenne)->moyenne de la temperature standard entre deux altitudes geometriques"
...
def construire_profil_temperature(max_altitude_km: float,step_m: float,) -> dict[str, np.ndarray]:
    ...
    "construire_profil_temperature(max_altitude_km,step_m)->dictionnaire contenant les listes d'altitudes(geometriques et geopotentielles), temperatures k_et_c et pourcentage pression"
...
"renvoie un csv et un graphique du profil de temperature standard"
...
"permet de tracer le graphique avec la température standard et de sauvegarder les données dans un CSV"
...
"fichier csv qui permet de tracer le graphique et donc contient les valeurs de la température en fct de l'alitutde"
    if args.csv:
