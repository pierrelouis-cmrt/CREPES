"""Profil vertical simplifie de la pression atmospherique et du CH4."""
...
def calculate_profile(
    ...
    ch4_ppm = surface_ch4_ppm + gradient_ppm_per_km * altitude_m / 1000.0
    if np.any(ch4_ppm <= 0.0):
        raise ValueError("Le profil de CH4 devient nul ou negatif.")

    ch4_mole_fraction = ch4_ppm * 1e-6
    return {
        ...
        "ch4_ppm": ch4_ppm,
        "ch4_partial_pressure_pa": pressure_pa * ch4_mole_fraction,
        "ch4_number_density_m3": pressure_pa
        * ch4_mole_fraction
        / (K_B * temperature_k),
    }
...
def build_plot(profile: dict[str, np.ndarray], use_file_backend: bool):
    ...
    axes[1].plot(profile["ch4_ppm"], altitude_km, color="darkorange")
    axes[1].set_xlabel("Rapport de melange CH4 (ppm)")
    axes[2].semilogx(profile["ch4_number_density_m3"], altitude_km, color="teal")
    axes[2].set_xlabel("Concentration CH4 (molecules/m3)")

    for axis in axes:
        axis.grid(True, which="both", alpha=0.3)

    fig.suptitle("Evolution verticale de la pression et du CH4")
    fig.tight_layout()
    return fig, plt
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Modele le profil vertical de la pression et du CH4."
    )
    parser.add_argument("--max-altitude-km", type=float, default=50.0)
    parser.add_argument("--step-m", type=float, default=100.0)
    parser.add_argument("--surface-ch4-ppm", type=float, default=1.90)  # ~1900 ppb
    parser.add_argument("--ch4-gradient-ppm-per-km", type=float, default=0.0)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.surface_ch4_ppm <= 0.0:
        raise ValueError("--surface-ch4-ppm doit etre strictement positif.")
    ...
    profile = calculate_profile(
        altitude_m,
        args.surface_ch4_ppm,
        args.ch4_gradient_ppm_per_km,
    ...
    if headless and output_path is None:
        output_path = SCRIPT_DIR / "profil_atmosphere_ch4.png"
