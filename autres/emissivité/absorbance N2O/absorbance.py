...
def make_cross_section_N2O_all_bands():
    warnings.filterwarnings("ignore", category=LinestrengthCutoffWarning)
   
    bands = [
        (500, 700),   # bande à 589 cm⁻¹ ~ 15 µm 
        (1100,1500),  # bande à 1285 cm⁻¹ ~ 7.2 µm
        (2100, 3000),  # bande à 2224 cm⁻¹ ~ 4.3 µm
    ...
    a= 336e-9  # fraction molaire du N2O (440 ppm)
    for wmin, wmax in bands:
        
        s = calc_spectrum(
            wmin=wmin, wmax=wmax,
            molecule="N2O", isotope="1,2,3",
            Tgas=255, pressure=1.013,
            mole_fraction=a,   # 336 ppm
...
absorption_N2O = make_cross_section_N2O_all_bands()

wl_um = np.linspace(4, 17, 10000)

fig, ax = plt.subplots(figsize=(10, 5))
ax.plot(wl_um, absorption_N2O(wl_um), color="steelblue", linewidth=0.8)
ax.set_xlabel("Longueur d'onde (µm)")
ax.set_ylabel("Absorbance ")
ax.set_title("Absorption du N2O (336 ppb, 1m)")
